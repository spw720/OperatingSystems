...
void alarm_handler(int signal){
  ...
  while (flag_boi > 0){
    ...
    if (w = waitpid(pid_pool[running_child], &wstatus, WNOHANG) != 0){
      ...
      if(last_cpu_time < int(stime) - last_stime_read[running_child]){
      ...
      if(last_IO_time < int(utime) - last_utime_read[running_child]){
        priorities[running_child] += 1;
        printf("[%d] IO intensive. Time quantum now: %d\n", process_id, priorities[running_child]);
      }

      //set arrays to be referenced on next iteration
      last_IO_time[running_child] = int(utime) - last_utime_read[running_child];
      last_cpu_time[running_child] = int(stime) - last_stime_read[running_child];
      last_utime_read[running_child] = int(utime);
      last_stime_read[running_child] = int(stime);
